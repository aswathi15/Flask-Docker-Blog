...
model = pickle.load(open('model_w2v.pkl', 'rb'))
...
weights = np.array(np.ones(len(y_train)))
...
clf_xgb = XGBClassifier(max_depth=10, scale_pos_weight=14,n_estimators=10,learning_rate=0.1, colsample_bytree=.7, gamma=0, reg_alpha=4, objective='multi:softprob', eta=0.3, silent=1, subsample=0.8).fit(X_train, y_train) 
...
print("F1-score: %1.3f" % f1_score(y_test,y_pred_xgb,average='macro'))
print("Precision: %1.3f" % precision_score(y_test,y_pred_xgb,average = 'macro'))
print("Recall: %1.3f" % recall_score(y_test,y_pred_xgb,average = 'macro'))
